...
- **Next checkpoint:** Finish the audit phase, confirm coverage of functions/schedules/storage/deployment, and then move into planning the schema/view adjustments. Baseline work is now captured in `mission-control/knowledge-base/KB-08-convex-baseline.md`; log each follow-up action here or in the KB so the next phase can begin without delay.
